...
import { HTTP400Error } from "../errors/AppErrors";
 ...
 */
export const validateRequestPayloadSchema = <T>(
    schema: AnyZodObject,
    payload: T
): T => {
    ...
    if (!(validatedData.error instanceof ZodError)) {
        ...
        throw new Error("Error while validating request payload");
    }
    ...
    throw new HTTP400Error(errorMessages);
};
